...
def start():
    ...
    if now1 != 10:
        ...
        coll = random.randrange(2, 3)
        starty = random.randrange(350)
        Duck(width - 150, starty, spx, spy, coll, all_sprites)
        Duck(0, starty, spx, spy, coll, all_sprites)
    elif now1 == 10:
        finall.final()
...
class Bullet(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if True in a13 and not lasttest and shoot1:
            a = pygame.sprite.spritecollideany(self, ducks)
            if a:
...
if __name__ == '__main__':
    ...
    Restart(all_sprites)
    sobaka = AnimatedSprite(load_image("dog3.png", 'data', colorkey=-1), 5, 1, 0, 630)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if key[pygame.K_r]:
                    ...
                    a50 = 0
        a1.tick(v)
        if sobaka.rect.x != 400:
            sobaka.rect.x += 5
        if times == 3:
            sobaka.kill()
            lasttest = False
            times = 4
        if sobaka.rect.x == 400 and jump:
            jump = False
            a4 = sobaka.rect.x
            times += 1
            sobaka.kill()
            sobaka = AnimatedSprite(load_image("jump2.png", 'data', colorkey=-1), 3, 1, a4, 600)
